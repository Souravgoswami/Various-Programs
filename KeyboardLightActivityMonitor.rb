...
def main(sleep: 0.05, clear: false)
	...
	if (cp || sw || st)
		unless clear		
			...
			while true
				cp && Calculate.get_cpu_usage > 50 ? n.each { |x| IO.write(x, 1) unless IO.read(x).to_i == 1 } : n.each { |x| IO.write(x, 0) unless IO.read(x).to_i == 0 }
				sw && Calculate.get_swap_usage > 0 ? c.each { |x| IO.write(x, 1) unless IO.read(x).to_i == 1 } : c.each { |x| IO.write(x, 0) unless IO.read(x).to_i == 0 }
				st && Calculate.get_disk_status ? s.each { |x| IO.write(x, 1) unless IO.read(x).to_i == 1 } : s.each { |x| IO.write(x, 0) unless IO.read(x).to_i == 0 }
				sleep sleep
			end
		else
